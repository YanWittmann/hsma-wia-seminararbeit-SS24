...
In den folgenden Unterkapiteln werden Muster wie von \autocite[S. 3]{Bach.DashboardDesignPatterns.2023} vorgestellt, unterschieden in konkrete Visualisierungselemente, Daten--Abstraktionsniveaus und Metainformationen.

\subsubsection{Visuelle Darstellungs-Patterns}

Die Wahl eines gewünschten Detailgrades der Daten (\autoref{subsubsec:data-information-patterns}) hat einen großen Einfluss darauf, welche der im Folgenden genannten Elemente infrage kommen.
...
In ähnlichem Stil zu \autoref{subsec:content-design-patterns} werden nun die Kompositionspatterns anhand von \autocite[S. 4--5]{Bach.DashboardDesignPatterns.2023} vorgestellt.
Diese beschreiben, wie die individuellen Komponenten mit ihren Titeln, Datenelementen und Beschreibungen (als Widgets bezeichnet) eines Dashboards angeordnet und gruppiert werden können, um eine sinnvolle Gesamtsicht zu erzeugen.
...
\begin{itemize}
    ...
    \item \emph{Gruppierte Layouts} fassen zusammengehörige Widgets visuell zusammen, z.B.\ durch gemeinsamen Titel, Rahmen, Hintergrundfarben oder Abstände.
\end{itemize}
...
\begin{itemize}
    ...
    \item \emph{Details auf Abruf}: Zusätzliche Inhalte werden z.B.\ durch Tooltips oder Popups bei Interaktion eingeblendet.
    ...
    \item \emph{Mehrere Seiten}: Inhalte werden auf mehrere Seiten aufgeteilt, die durch Navigations-Patterns erreichbar sind.
\end{itemize}

\subsubsection{Struktur-Patterns}

% Struktur-Patterns erfassen Beziehungen zwischen mehreren Seiten eines Dashboards:

% \begin{itemize}
%     \item \emph{Einzelseite} (61\%): Das Dashboard besteht aus einer einzigen Seite.
%     \item \emph{Hierarchie} (14\%): Seiten sind hierarchisch angeordnet, oft mit einer Startseite und Unterseiten.
%     \item \emph{Sequenz} (11\%): Seiten bilden eine lineare Sequenz, ähnlich einer Präsentation oder eines Assistenten.
%     \item \emph{Netzwerk} (6\%): Seiten sind netzwerkartig verbunden und ermöglichen eine freie Navigation.
%     \item \emph{Reiter} (8\%): Ähnliche Ansichten mit unterschiedlichen Daten werden in Reitern organisiert.
% \end{itemize}
